...
public:
	...
	void begin(const char* name)
	{
		...
		ImGui::NewFrame();
		windows.push_back(ImGui::Begin(name));
	}
	void end()
	{
		ImGui::End();
	...
	void CreateButton(const char* name, vec2 size)
	{
		buttons.push_back(ImGui::Button(name, ImVec2(size.x, size.y)));
	}
...
int main(int argc, char* argv[])
{
	...
	gui* Gui = new gui(window.getWindow());
	...
	float positions[3];
	bool s1DropDown = false;
	while (!window.ShouldClose())
	{

		Gui->begin("Object List");
		if (ImGui::Button("Square 1", ImVec2(50, 100))) s1DropDown = !s1DropDown;
		if (s1DropDown) ImGui::Text("works");
		Gui->end();
